    ...
    pub async fn run(
        ...
        match &self.command {
                ...
                */
                ...
                let pool_info = loop {
                    match contracts.compute_pool.get_pool_info(pool_id).await {
                        ...
                        Ok(_) => {
                            Console::warning(
                                "Pool is not active yet. Checking again in 15 seconds.",
                            );
                            tokio::select! {
                ...
                if let Some(ref compute_specs) = node_config.compute_specs {
                    ...
                    Console::warning(
                        "Cannot verify compute requirements: node specs not available.",
                    );
                    if !*skip_system_checks {
                        std::process::exit(1);
                    } else {
                        ...
                        Ok(res) => {
                            match res {
                                ...
                                _ => {
                                    ...
                                    error!(
                                        "❌ Unexpected result from putting record in DHT: {res:?}"
                                    );
                                    std::process::exit(1);
                                }
                            }
                ...
                while let Some(result) = result_rx.recv().await {
                    match result {
                        Ok(res) => {
                            match res {
                                p2p::KademliaQueryResult::StartProviding(res) => {
                                    match res {
                                        Ok(_) => {
                                            Console::success(
                ...
                if let Err(err) =
                    compute_node_ops.start_monitoring(cancellation_token.clone(), pool_id)
                {
            ...
            Commands::Balance {
                ...
                let provider_wallet =
                    Wallet::new(&private_key, Url::parse(rpc_url).unwrap()).unwrap();
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        Console::user_error(&format!(
                            "❌ Failed to check if compute node exists: {e}"
                        ));
                        std::process::exit(1);
                    }
                };
