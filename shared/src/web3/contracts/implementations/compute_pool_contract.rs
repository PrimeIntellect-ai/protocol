...
impl ComputePool {
    ...
    pub async fn get_pool_info(&self, pool_id: U256) -> Result<PoolInfo, Box<dyn std::error::Error>> {
        let pool_info_response = self.instance.instance().function("getComputePool", &[pool_id.into()])?.call().await?;
        let pool_info_tuple: &[DynSolValue] = pool_info_response.first().unwrap().as_tuple().unwrap();
    ...
    pub async fn start_compute_pool(&self, pool_id: U256) -> Result<FixedBytes<32>, Box<dyn std::error::Error>> {
        let result = self.instance.instance().function("startComputePool", &[pool_id.into()])?.send().await?.watch().await?;
        Ok(result)
    }
    ...
    pub async fn join_compute_pool(
        ...
        let address = DynSolValue::from(nodes.iter().map(|addr| DynSolValue::from(*addr)).collect::<Vec<_>>());
        let signatures = DynSolValue::from(signatures.iter().map(|sig| DynSolValue::Bytes(sig.to_vec())).collect::<Vec<_>>());
